...
public class LocalFileManager {
	...
	public static void download(SendObject s) {
		String filePath = UserProperties.getDirectory() + pathParser(s.getFilePath())
				+ s.getFileName();
	...
	public static void rename(SendObject s) {
		...
		String oldName = UserProperties.getDirectory() + pathParser(s.getFilePath())
				+ s.getFileName();
		System.out.println("Old name is " + oldName);
		String newName = UserProperties.getDirectory() + pathParser(s.getFilePath())
				+ s.getNewFileName();	
	...
	/*public static void main(String[] args) {
		...
		s.setFileName("poos.mp3");
		//s.setNewFileName("poos.mp3");
		delete(s);
		//rename(s);
	} */
}
